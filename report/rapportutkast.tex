...
En annan viktig anledningen till att vi använder Django är att det är bra i säkerhetsavseende. Django kommer med väldigt många olika säkerhetsåtgärder inbyggt och det finns mycket säkerhetsrelaterad information dokumenterat på deras hemsida. Dessa inbyggda funktioner kan vi använda oss av för att göra säkerheten på hemsidan så bra som möjligt \cite{securityInDjango}. Bland de säkerhetsåtgärder som Django erbjuder automatiskt så finns det två som är viktiga för oss. Django är automatiskt skyddat för SQL-injection %TODO referens
...
Vi har ett fullt fungerande system som kan visa vart en medarbetare befinner sig inom ett område som har blivit nätverksskannad innan. Personen ska även finnas med i vår databas där alla referenspunkter och användare är sparade. Medarbetarnas position visas i en webapplikation där man även kan välja att inte delge sin egen position. Vi kan uppskatta en medarbetares position med en radie på \ldots %TODO fyll i med verkliga siffror
