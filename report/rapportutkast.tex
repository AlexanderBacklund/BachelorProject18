...
\subsection{Recieved Signal Strength Indication - RSSI}
RSSI är ett mått på signalstyrkan från en trådlös åtkomstpunkt. RSSI mäts i enheten decibel(dB) och anger hur mycket styrkan av en signal har avtagit innan den signalen uppfångas av en mottagarenhet.
%\TODO: referenser och fortsätta och fortsaätta

\subsection{Referenspunkter i samband med Fingerprintingmetoden}
...
En referenspunkt bestäms som en specifik position i en lokal där man gör mätningar av den trådlösa nätverksinformationen. Det som analyseras vid dessa mätningar är vilka trådlösa åtkomstpunkter, vanligvis routrar, som går att ansluta till vid referenspunktens position. För varje åtkomstpunkt som upptäcks vid analysen av den trådlösa nätverksmiljön mäts även signalstyrkan till respektive åtkomstpunkt. Signalstyrkan ges i form av ett RSSI-värde. När referenspunkterna utmäts sparar man information om ett visst antal åtkomstpunkter vid den specifika referenspunkten samt RSSI-värdet för respektive åtkomstpunkt\cite{IP1}. Vanligtvis sparas information om de åtkomstpunkter som anses befinna sig närmast referenspunkten, detta baserat på RSSI-värdet för respektive åtkomstpunkt\cite{IP1}. Denna information i kombination med information om vart i lokalen referenspunkten är utmätt lagras i en databas. Referenspunkter utmäts vanligvis i ett rutmönster med ett par meters mellanrum i alla de lokaler där man vill kunnna utföra inomhuspositionering\cite{IP1}. När mätningar är utförda vid varje referenspunkt finns all information som behövs om nätverksmiljön i en byggnad lagrad i en databas. Det är denna information som sedan används som referenser till olika platser i lokalerna för att kunna positionsbestämma enheter som sedan befinner sig i byggnaden\cite{yiu2017wireless}\cite{IP1}.
...
\subsection{Hemsida för presentation av positionsinformation}
...
\begin{itemize}
  ...
  \end{itemize}
\texitit{Id}-fältet är ett index som är unikt för varje referenspunkt. Området anger vart denna referenspunkt befinner sig, alltså vart nätverksanalysen är genomförd. \textit{MAC 1}-fältet håller MAC-adressen för den åtkomstpunkt vars RSSI-värde var störst vid nätverksanalysen. Det tillhörande RSSI-värdet ligger i fältet \textit{RSSI MAC 1}.
