...
\section{Skript för positionsbestämning}
...
\subsection{Algoritm för positionsbestämning}
...
Metod 1 bygger på teorin att de referenspunkter som har åtkomstpunkter sparade som överensstämmer med de åtkomstpunkter med högst RSSI-värde i klienenhetens lista (se exempel i sektion \ref{scanning}) är de referenspunkter som befinner sig närmast klientenheten. Om det går att finna en referenspunkt vars tre åtkomstpunkter återfinns som top tre åtkomstpunkter i klientenhetens lista bör denna refeferenspunkt i teorin befinna sig i nära anslutning till klientenheten.
...
Algoritmen för metod 1, med syfte att uppskatta vilken referenspunkt som befinner sig närmast klientenheten ser ut som följer:
  ...
  \item \textbf{Steg 4}
  \newline
  Om fler än en referenspunkt slutar på samma poäng i steg 3 av algoritmen betyder det att vi inte bara med hjälp av att undersöka den inbördes ordningen för åtkomstpunkterna kan avgöra vilken som teoretiskt sett bör vara den som bäst överensstämmer med klientenhetens position. Vid de fall som detta uppstår vill vi ändå kunna utse en referenspunkt som den bäst matchande. Det gör vi genom att övergå till Metod 2 för positionsbestämning.
...
I de fall som Metod 1 för positionsbestämningen av en klientenhet inte har lyckats avgöra vilken referenspunkt som bäst matchar klientenhetens position behöver vi ytterligare en metod för att utse \textbf{en} specifik referenspunk som den bäst matchande. Om Metod 1 inte kunnat fastställa en specifik referenspunkt betyder det att två eller fler kandiderar om att vara bäst matchande. Dessa kandidater skickas vidare till Metod 2.
